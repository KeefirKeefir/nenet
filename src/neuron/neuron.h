...
#define BASIC_NEURON_COUNT                                                     \
  (NEURON_COUNT - SENSOR_NEURON_COUNT - PACEMAKER_NEURON_COUNT)
...
static_assert(SENSOR_END == NEURON_COUNT,
              "neuron ranges should add up to the neuron count");

// whether a neuron is a sensor, pacemaker, excitor, or inhibitor is determined
...
typedef struct neuron_t {
  ...
  uchar pulse_timer; // counts down, when it reaches input_strength, the neuron
                     // pulses, adding +-2 charge to each target

  uchar input_strength; // 0 means no sensory input, timer counts down to this,
                        // then pulses
...
neuron_t *new_neuron_array() {
  ...
  for (neuron_t *it = neurons, *end = neurons + NEURON_COUNT; it < end;
       it++, i++) {
    for (uint32 *it2 = it->targets, *end2 = it->targets + NEURON_TARGET_COUNT;
         it2 < end2; it2++) {
      // printf("%u ", *it2);
...
void fire_excitor(neuron_t *neuron, neuron_t *neurons) {
  for (uint32 *it = neuron->targets,
              *end = neuron->targets + NEURON_TARGET_COUNT;
       it < end; it++) {
    neurons[*it].charge = saturated_add_uchar(neurons[*it].charge, 5);
    if (neurons[*it].charge == 255) {
      neurons[*it].pulse_counter =
          saturated_add_uchar(neurons[*it].pulse_counter, 1);
      neurons[*it].charge = 0;
    }
  }
...
void try_fire_sensor(neuron_t *neuron, neuron_t *neurons) {
  if (neuron->input_strength > 0 &&
      neuron->pulse_timer <= neuron->input_strength) {
    fire_excitor(neuron, neurons);
    neuron->pulse_timer = 255;
  }
}
...
void neuron_tick(neuron_t *neurons) {
  ...
  void *current_neuron_type_label =
      &&EXCITOR_LABEL; // g_neuron_index love computed goto
